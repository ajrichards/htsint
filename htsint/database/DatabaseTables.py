...
class Refseq(Base):
    '''
    class that handles Refseq
    '''

    __tablename__ = 'refseq'

    id = Column(Integer, Sequence('refseq_id_seq'),primary_key=True)
    status = Column(String)
    RNA_nucleotide_accession = Column(String)
    RNA_nucleotide_gi = Column(String)
    protein_accession = Column(String)
    protein_gi = Column(String)
    genomic_nucleotide_accession = Column(String) 
    genomic_nucleotide_gi = Column(String) 
    start_position_on_the_genomic_accession = Column(String) 
    end_position_on_the_genomic_accession = Column(String) 
    orientation = Column(String) 
    assembly = Column(String) 
    mature_peptide_accession = Column(String)
    mature_peptide_gi = Column(String)
    taxa_id = Column(Integer,ForeignKey('taxa.id'),nullable=False)
    gene_id = Column(Integer,ForeignKey('genes.id'),nullable=True)

    def __init__(self,status,RNA_nucleotide_accession,RNA_nucleotide_gi,protein_accession,
                 protein_gi, genomic_nucleotide_accession, genomic_nucleotide_gi,
                 start_position_on_the_genomic_accession,end_position_on_the_genomic_accession, 
                 orientation, assembly,mature_peptide_accession,mature_peptide_gi,
                 taxa_id,gene_id):

        self.status = status
        self.RNA_nucleotide_accession = RNA_nucleotide_accession
        self.RNA_nucleotide_gi = RNA_nucleotide_gi
        self.protein_accession = protein_accession
        self.protein_gi = protein_gi
        self.genomic_nucleotide_accession = genomic_nucleotide_accession
        self.genomic_nucleotide_gi = genomic_nucleotide_gi
        self.start_position_on_the_genomic_accession = start_position_on_the_genomic_accession
        self.end_position_on_the_genomic_accession = end_position_on_the_genomic_accession
        self.orientation = orientation
        self.assembly = assembly
        self.mature_peptide_accession = mature_peptide_accession
        self.mature_peptide_gi = mature_peptide_gi
        self.taxa_id = taxa_id
        self.gene_id = gene_id

    def __repr__(self):
        return "<Refseq('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')>"%(self.status, 
                                                                                                       self.RNA_nucleotide_accession,
                                                                                                       self.RNA_nucleotide_gi,
                                                                                                       self.protein_accession,
                                                                                                       self.protein_gi,
                                                                                                       self.genomic_nucleotide_accession,
                                                                                                       self.genomic_nucleotide_gi,
                                                                                                       self.start_position_on_the_genomic_accession,
                                                                                                       self.end_position_on_the_genomic_accession,
                                                                                                       self.orientation,
                                                                                                       self.assembly,
                                                                                                       self.mature_peptide_accession,
                                                                                                       self.mature_peptide_gi,
                                                                                                       self.taxa_id,
                                                                                                       self.gene_id)
